class FuzzyMatcher {
  ...
  static String _normalizeAndStem(String word) {
    ...
    w = _stripSuffix(w, ruNoun);

    return w;
  }

  /// Как replaceFirst(suffix, ''), но никогда не съедает слово целиком.
  /// Однобуквенное слово вроде "и" само по себе совпадает с одной из
  /// однобуквенных альтернатив ruNoun (а|я|у|ю|е|о|ы|и|ь) — без этой защиты
  /// оно стеммилось до пустой строки, isMatch() трактовал пустой список слов
  /// запроса как "запрос пуст" и матчил вообще все объявления.
  static String _stripSuffix(String word, RegExp suffix) {
    final stripped = word.replaceFirst(suffix, '');
