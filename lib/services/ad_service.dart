    ...
    try {
      ...
      if (video != null) {
        ...
        if (videoUrl != null) {
          ...
          // Как и с фото (см. выше): FileService.uploadFile после 3 неудачных
          // попыток возвращает null, а не бросает исключение. Раньше это
          // молча продолжало выполнение с videoUrl == null, который потом
          // безусловно уходил в AdModel.toMap() → .update() и СТИРАЛ
          // video_url в Firestore — тот же класс бага, что и 6ce8801, но
