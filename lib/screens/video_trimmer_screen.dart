...
class _VideoTrimmerScreenState extends State<VideoTrimmerScreen> {
  ...
  Future<void> _saveVideo() async {
    ...
    try {
      ...
      // то есть лимит по факту не соблюдался.
      //
      // Проверяем ОБЕ границы, а не только старт. Раньше было только
      // "_startTime > 0" — для ролика короче лимита (например, 10 сек) это
      // пропускало обрезку, если пользователь двигал только ПРАВЫЙ маркер
      // (сузил выделение с 10 сек до, скажем, 4 сек, не трогая левый край).
      // needsTrim оставался false, videoCompress получал startTime/duration:
      // null и жал ролик ЦЕЛИКОМ — все 10 сек вместо выбранных 4, то есть
      // обрезка внутри лимита молча игнорировалась.
      final bool needsTrim = _startTime.inMilliseconds > 0 ||
          _endTime.inMilliseconds < _totalDuration.inMilliseconds;
